import { Patient, Placement, Department } from '../models';
...
const initialDepartments: Department[] = [
  ...
  },
  {
    ...
    rooms: [
      ...
      { id: 'r7', number: '302', capacity: 3, status: 'closed' },
    ],
  },
];

let placements: Placement[] = deepCopy(initialPlacements);
let departments: Department[] = deepCopy(initialDepartments);
...
export function updateDepartment(department: Department): Department {
  ...
  if (index >= 0) {
    departments[index] = { ...department, rooms: department.rooms.map(r => ({ ...r })) };
...
export function getOccupiedBeds(departmentId: string): number {
  return placements.filter(p => p.departmentId === departmentId).length;
}
